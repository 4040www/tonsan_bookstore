...
export default function To_do_list() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                let filteredData;
                if (filter === 'all') {
                    filteredData = data.filter(item => item.source_table === 'eventsTodo').filter(item => item.status !== 1);
                } else {
                    filteredData = data.filter(item => item.source_table === 'eventsTodo').filter(item => item.status === filter);
                }
                setToDoData(order ? filteredData.reverse() : filteredData);
            } catch (error) {
    ...
    const handleSaveClick = async () => {
        try {
            ...
            const response = await eventsToDo(newTask);
            alert('已成功增加待辦事項');
            console.log('Fetching data from API...');
            const data = await Overview();
            console.log('Data received from API:', data);
    ...
    const EditCard = ({ task }) => {
        ...
        const handleEditSaveClick = async () => {
            try {
                ...
                const response = await editEventsToDo(editingTask, dataId);
                alert('已成功編輯待辦事項');
                console.log('Fetching data from API...');
                const data = await Overview();
                console.log('Data received from API:', data);
    ...
    return (
        <>
            <div className="title-container-6">
                <img src={todo} className="icon-in-page" />
                <h1>活動待辦事項</h1>
            </div>
