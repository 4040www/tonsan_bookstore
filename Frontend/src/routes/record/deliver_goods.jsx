...
export default function Deliver_goods() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                let filteredData;
                if (filter === 'all') {
                    filteredData = data.filter(item => item.source_table === 'restockandrefund').filter(item => item.status !== 1);
                } else {
                    filteredData = data.filter(item => item.source_table === 'restockandrefund').filter(item => item.status === filter);
                }
                setStockData(order ? filteredData.reverse() : filteredData);
            } catch (error) {
    ...
    const handleSaveClick = async () => {
        try {
            ...
            const response = await restockAndRefund(newTask);
            alert('已成功增加待辦事項');
            console.log('Fetching data from API...');
            const data = await Overview();
            console.log('Data received from API:', data);
    ...
    const handleDeleteClick = async (item) => {
        ...
        try {
            await deleteDeliverOrder(item.id);
            alert('已成功刪除待辦事項');
            const data = await Overview();
            let filteredData;
            if (filter === 'all') {
    ...
    const EditCard = ({ task }) => {
        ...
        const handleEditSaveClick = async () => {
            try {
                ...
                const response = await editDeliverOrder(editingTask, dataId);
                alert('已成功編輯待辦事項');
                console.log('Fetching data from API...');
                const data = await Overview();
                console.log('Data received from API:', data);
    ...
    return (
        <>
            <div className="title-container-3">
                <img src={cart} className="icon-in-page" />
                <h1>進退貨</h1>
            </div>
