...
export default function River_order() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                let filteredData;
                if (filter === 'all') {
                    filteredData = data.filter(item => item.source_table === 'sinica').filter(item => item.status !== 1);
                } else {
                    filteredData = data.filter(item => item.source_table === 'sinica').filter(item => item.status === filter);
                }
                setRiverData(order ? filteredData.reverse() : filteredData);
            } catch (error) {
    ...
    const handleSaveClick = async () => {
        try {
            ...
            const response = await createRiverOrder(newTask);
            alert('已成功增加待辦事項');
            console.log('Fetching data from API...');
            const data = await Overview();
            console.log('Data received from API:', data);
    ...
    const handleDeleteClick = async (item) => {
        ...
        try {
            await deleteRiverOrder(item.id);
            alert('已成功刪除待辦事項');
            const data = await Overview();
            let filteredData;
            if (filter === 'all') {
    ...
    const EditCard = ({ task }) => {
        ...
        const handleEditSaveClick = async () => {
            try {
                ...
                const response = await editRiverOrder(editingTask, dataId);
                alert('已成功編輯待辦事項');
                console.log('Fetching data from API...');
                const data = await Overview();
                console.log('Data received from API:', data);
    ...
    return (
        <>
            <div className="title-container-3">
                <img src={water} className="icon-in-page" />
                <h1>四分溪</h1>
            </div>
