...
export default function Internet_order() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                let filteredData;
                if (filter === 'all') {
                    filteredData = data.filter(item => item.source_table === 'onlineorders').filter(item => item.status !== 1);
                } else {
                    filteredData = data.filter(item => item.source_table === 'onlineorders').filter(item => item.status === filter);
                }
                setOnlineData(order ? filteredData.reverse() : filteredData);
            } catch (error) {
    ...
    const handleSaveClick = async () => {
        try {
            ...
            const response = await createOrder(newTask);
            alert('已成功增加待辦事項');
            console.log('Fetching data from API...');
            const data = await Overview();
            console.log('Data received from API:', data);
    ...
    const EditCard = ({ task }) => {
        ...
        const handleEditSaveClick = async () => {
            try {
                ...
                const response = await editOnlineOrder(editingTask, dataId);
                alert('已成功編輯待辦事項');
                console.log('Fetching data from API...');
                const data = await Overview();
                console.log('Data received from API:', data);
    ...
    return (
        <>
            <div className="title-container">
                <img src={online} className="icon-in-page" />
                <h1>網路訂單</h1>
            </div>
